// GiáThành Pro — Cloud Server (Glitch.com)
const http=require('http'),fs=require('fs'),path=require('path'),os=require('os');

const PORT=process.env.PORT||3000;
// Glitch lưu data bền vững trong thư mục .data
const DATA_DIR=process.env.PROJECT_DOMAIN?'/app/.data':__dirname;
const DATA=path.join(DATA_DIR,'giathanh_data.json');
const HTML=path.join(__dirname,'giathanh_pro.html');
...
http.createServer((req,res)=>{
  ...
  if(url==='/'||url==='/index.html'){
    fs.readFile(HTML,(err,data)=>{
      if(err){res.writeHead(404);res.end('<h2>Không tìm thấy giathanh_pro.html</h2>');return;}
